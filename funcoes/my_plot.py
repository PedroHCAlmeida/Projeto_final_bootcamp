...
def annot_bar(data, ax, name_col_prop='Prop', hue_name='value'):
    rects = ax.patches
    for rect, row in zip(rects, data.iterrows()):
        height = rect.get_height()
        ax.text(rect.get_x() + rect.get_width() / 2, height, str(round(height * 100,2)) + '%', color='black', ha='center', va='bottom', fontsize=15)